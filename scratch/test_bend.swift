...
struct BendedCornerShape: InsettableShape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        if bendAmount > 0.01 {
            let cx = buttonOffset.x + buttonSize / 2
            let cy = buttonOffset.y + buttonSize / 2
            let cr = (buttonSize / 2) + padding + insetAmount
            ...
            if termLeft > 0 && termTop > 0 {
                ...
                let ctrlX = insetAmount + (cx - insetAmount) * bendAmount * 1.5
                let ctrlY = insetAmount + (cy - insetAmount) * bendAmount * 1.5
