...
class VolumeManager {
    ...
    func getCurrentAudioDeviceName() -> String {
        ...
        let nameStatus = withUnsafeMutablePointer(to: &deviceName) { ptr in
            AudioObjectGetPropertyData(defaultOutputDeviceID, &address, 0, nil, &deviceNameSize, ptr)
        }
        
        if nameStatus == noErr, let name = deviceName as String? {
            return name
        }
    ...
    func getCurrentInputDeviceName() -> String {
        ...
        let nameStatus = withUnsafeMutablePointer(to: &deviceName) { ptr in
            AudioObjectGetPropertyData(defaultInputDeviceID, &address, 0, nil, &deviceNameSize, ptr)
        }
    ...
    private func startMuteEnforcer() {
        if muteEnforcerTimer == nil {
            DispatchQueue.main.async { [weak self] in
                guard let self = self else { return }
                self.muteEnforcerTimer = Timer.scheduledTimerInCommonModes(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
                    guard let self = self else { return }
                    if self.isMicMuted {
    ...
    private func setCurrentInputDeviceVolume(volume: Float, mute: Bool) {
        ...
        AudioObjectSetPropertyData(device, &systemVolAddress, 0, nil, UInt32(MemoryLayout<Float>.size), &vol)
